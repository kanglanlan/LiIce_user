 ...
 *         created at 2018/8/17 15:48
 */
public class MyOrders_A extends BaseActivity {
    String[] quxdatas;
    ...
    private String[] titles = new String[]{"全部", "待付款", "待送货安装","待确认"};
    private List<Fragment> mList_Fragments = new ArrayList<>();
    private int orderType = 0; //0 全部 1  待付款 2  待送货安装
...
String status="0";
    ...
    private void initView() {
        ...
        for (int i = 0; i < titles.length; i++) {
            tablayoutMo.addTab(tablayoutMo.newTab().setText(titles[i]));
        }
        vpMo.setAdapter(new CustomAdapter(getSupportFragmentManager(), baseContext));
        ...
        CommonUtil.reflex(tablayoutMo, 10);
        switch (orderType) {
            case 1:
                tablayoutMo.getTabAt(1).select();
                break;
            case 2:
                tablayoutMo.getTabAt(2).select();
                break;
            case 0:
                getData(true);
            default:
...
//                                            getData(false);
    ...
    public void showreson(final String[] string, final String id, final int position) {
        ...
        tv_queding.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                dialog.dismiss();
                Nonce.order(string[wv1.getCurrentItem()], id, "1", baseContext, new Nonce.StringCallback() {
                    @Override
                    public void doWork(String string) {
